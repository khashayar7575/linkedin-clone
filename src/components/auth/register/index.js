...
const Register = () => {
  ...
  const profilePicURLChangeHandler = (event) => {
    setProfilePicUrl(event.target.value);
  };
  const formSubmitHanlder = (e) => {
    e.preventDefault();
    const dEmail = email.trim();
    const dPassword = password.trim();
    const dProfilePicUrl = profilePicUrl.trim();
    const dFullName = fullName.trim();
    if (!dEmail.includes("@") || dPassword.length < 6) {
      alert("Email not valid or password length less than 6");
      return;
    }
    if (dFullName.length < 3 || !dProfilePicUrl.includes("http")) {
  ...
  return (
    <div className={styles.container}>
      ...
      <form className={styles.form} onSubmit={formSubmitHanlder}>
        <Input
        ...
        <Input
          ...
          type="text"
          value={profilePicUrl}
          onChange={profilePicURLChangeHandler}
        />
        <Input
          placeholder="Email"
          type="email"
      ...
      <div className={styles.changeMod}>
        <p>
          Already on Linkedin?
          <span onClick={signInClickHandler}>Sign in</span>
        </p>
      </div>
    </div>
  );
